...
export default function DashboardContent() {
  ...
  // Mode and query parameters
  const mode = searchParams.get('mode') === 'compare' ? 'compare' : 'analyze';
  const username = searchParams.get('username') || null;
  const user1 = searchParams.get('user1') || '';
  const user2 = searchParams.get('user2') || '';

  const [u1Input, setU1Input] = React.useState(user1);
  const [u2Input, setU2Input] = React.useState(user2);
  ...
  // Fetch data using SWR (Only for single user mode)
  const {
    user,
    error: userError,
    isLoading: userLoading,
    mutate: mutateUser,
  } = useGitHubUser(mode === 'analyze' ? username : null, token);
  ...
  const {
    ...
    mutate: mutateRepos,
  } = useGitHubRepos(mode === 'analyze' ? username : null, token);
  ...
  const handleModeChange = (newMode: 'analyze' | 'compare') => {
    const params = new URLSearchParams();
    if (newMode === 'compare') {
      params.set('mode', 'compare');
      if (u1Input) params.set('user1', u1Input);
      if (u2Input) params.set('user2', u2Input);
    } else {
      if (username) {
        params.set('username', username);
      } else if (u1Input) {
        params.set('username', u1Input);
      }
    }
    router.push(`?${params.toString()}`);
  };
  ...
  const handleCompareSubmit = (e?: React.FormEvent) => {
    ...
    const params = new URLSearchParams();
    params.set('mode', 'compare');
    if (u1Input.trim()) params.set('user1', u1Input.trim());
    if (u2Input.trim()) params.set('user2', u2Input.trim());
    router.push(`?${params.toString()}`);
  };
    ...
    <div className="flex-1 flex flex-col min-h-screen bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-50 font-sans transition-colors duration-200">
      ...
      <main className="flex-1 container max-w-6xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-8">
        ...
        <div className="flex justify-center">
          <div className="inline-flex p-1 bg-neutral-100 dark:bg-neutral-900 rounded-2xl border border-neutral-200/50 dark:border-neutral-800/40 shadow-sm">
            <button
        ...
        <div className="w-full text-center space-y-4">
          {mode === 'analyze' && !username && (
            <div className="space-y-2 max-w-2xl mx-auto py-4">
          ...
          {mode === 'compare' && !user1 && !user2 && (
            <div className="space-y-2 max-w-2xl mx-auto py-4">
              ...
              <p className="text-sm sm:text-base text-neutral-500 dark:text-neutral-400 font-medium">
                Compare profiles side-by-side: track followers, check programming languages, map repositories, and find out who wins the developer stat battle.
              </p>
            </div>
          )}
          ...
          {mode === 'analyze' ? (
            ...
            <form onSubmit={handleCompareSubmit} className="flex flex-col sm:flex-row items-center gap-3.5 max-w-2xl mx-auto z-40 relative">
              <div className="relative flex-1 w-full">
                ...
                <Input
                  ...
                  value={u1Input}
                  onChange={(e) => setU1Input(e.target.value)}
                  className="w-full pl-11 pr-4 py-6 text-base bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-lg focus-visible:ring-2 focus-visible:ring-indigo-500 dark:focus-visible:ring-indigo-400 focus-visible:border-transparent transition-all"
                />
              </div>
              <div className="text-xs font-black text-neutral-400 dark:text-neutral-500 select-none px-1">VS</div>
              <div className="relative flex-1 w-full">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-neutral-400 dark:text-neutral-500" />
                <Input
                  type="text"
                  ...
                  value={u2Input}
                  onChange={(e) => setU2Input(e.target.value)}
                  className="w-full pl-11 pr-4 py-6 text-base bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-lg focus-visible:ring-2 focus-visible:ring-indigo-500 dark:focus-visible:ring-indigo-400 focus-visible:border-transparent transition-all"
                />
              </div>
              <Button
                type="submit"
                disabled={!u1Input.trim() || !u2Input.trim()}
                className="w-full sm:w-auto px-6 py-6 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold shadow-lg hover:shadow-xl dark:bg-indigo-500 dark:hover:bg-indigo-600 hover:scale-[1.02] active:scale-[0.98] transition-all shrink-0 cursor-pointer"
              >
                Compare
              </Button>
            </form>
